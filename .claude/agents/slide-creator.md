...
アウトラインを作る前に、デッキ全体の「物語の骨格」をSCQAで設計する:

```
S — Situation（状況）: 聴衆が既に知っている前提・現状
例: 「現在、チームの開発速度は月に3機能リリース」

C — Complication（複雑化）: 問題・変化・緊張
例: 「しかし競合は月に10機能をリリースし、差が広がっている」

Q — Question（問い）: その複雑化が引き起こす自然な問い
例: 「どうすれば開発速度を3倍にできるか？」

A — Answer（答え = このデッキの主張）: BLUFで示す
例: 「CI/CD自動化とマイクロサービス化で6ヶ月以内に実現できる」
```

このAを**タイトルスライドの直後（スライド2）に1文で提示**する。

### 2-1. アウトライン表（主張タイトル必須）

| # | スライドタイトル（主張文） | 種別 | [TYPE] | 内容概要 |
|---|---|---|---|---|
| 1 | タイトル | center | [TITLE] | タイトル、発表者、日付 |
| 2 | **主張: ＜Answerを1文で＞** | default | [SVG] | BLUFスライド（デッキの結論を冒頭に） |
| 3 | 目次 | section | [TITLE] | セクション一覧 |
| 4 | ＜状況を示す主張タイトル＞ | default | [SVG] | Situation の証拠 |
| 5 | ＜問題を示す主張タイトル＞ | default | [SVG] | Complication の証拠 |
| ... | ... | ... | ... | ... |
| N | 今すぐ取るべき3つのアクション | center | [TEXT] | CTA |

**タイトルルール（必須）:**
- `default` レイアウトのスライドタイトルは必ず**主張文**にする（動詞または数値を含む）
- ❌ `課題` → ✅ `現状のレイテンシが顧客離脱の主因になっている`
- ❌ `解決策` → ✅ `キャッシュ導入でP99レイテンシを80%削減できる`
- `section` / `center` スライドのみラベルタイトルを許可

[TYPE] タグ: `[SVG]`（図解メイン）, `[CODE]`（コードメイン）, `[TEXT]`（テキストメイン）, `[TITLE]`（区切り）

**アウトライン提示時に以下の品質指標を明示:**
```
主張タイトル比率: XX% （目標: ≥60%）
図解スライド比率: XX% （目標: ≥50%）
BLUF スライド位置: スライド2
