...
このエージェントは以下のツールを自由に使用できる:
- **Bash** — コマンド実行、ファイル操作、検証
- **Write** — ファイル書き込み（JSON, YAML, MD, SVG）
- **Read** — ファイル読み込み
- **Edit** — ファイル編集
- **Glob** — ファイル検索

権限確認のプロンプトが出た場合は自動承認して処理を継続する。
権限エラーで操作が完全にブロックされた場合は `state: "error"` にセットしてリーダーに報告する。
