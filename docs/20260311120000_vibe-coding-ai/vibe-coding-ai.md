...
<!--
インストール〜初回起動の手順。/init コマンドでプロジェクト専用の CLAUDE.md を自動生成できる。
-->
...
<!--
典型的なClaude Codeコマンド例。-p フラグでプロンプトを指定するワンショット実行が CI/CDで特に便利。
-->
...
<!--
CLAUDE.mdは会話コンテキストに含めずAIが自動参照できる外部知識ベース。プロジェクト固有のルールを集約する。
-->
...
<!--
.claude/settings.json でツール実行の許可・拒否リストとフック自動化を定義できる。Write/Edit後の自動フォーマットが特に有用。
-->
...
<!--
Claude Codeへの指示は自然言語で書く。コンテキストを明確に伝えることで、既存コードの構造を理解した上で整合性のある実装が得られる。
-->
...
<!--
バグ修正時はエラーメッセージやスタックトレースを直接貼り付けると、Claude Codeが原因を特定して修正案を提示する。修正後の再現テスト追加も依頼できる。
-->
...
<!--
テスト自動生成はClaude Codeが最も得意とする用途の一つ。既存コードを読み込んで、関数の動作を理解した上で意味のあるテストケースを生成する。
-->
