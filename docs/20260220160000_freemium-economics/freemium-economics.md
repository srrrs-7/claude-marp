...
> *8つの章で無料戦略の経済学・設計・失敗パターン・実装を網羅する*
...
# 無料ユーザーのコストは本当にゼロか（2/2）

> *インフラ・サポート・機会コストを加算すると無料ユーザーコストは月数ドルになる*
...
# LTV 方程式 — チャーンレートとの関係（2/2）

> *月次チャーン1%と5%ではLTVが5倍以上乖離する—チャーン管理の優先度*
...
> *機能制限・使用量制限・ペインポイント解消の3レバーで転換率を2倍にできる*
...
# Spotify — 音楽フリーミアムの教科書（2/2）

> *Spotifyの30%転換率はペルソナ別制限設計と家族プランで達成された*
...
# Slack — B2B PLG（Product-Led Growth）の手本（2/2）

> *SlackのPLG戦略はボトムアップ導入で平均ARPUを従来SaaSの3倍に高めた*
...
# Dropbox — バイラルループと容量制限（2/2）

> *Dropboxの紹介プログラムが獲得コストを$0に近づけバイラル成長を実現した*
...
# Zoom — コロナが証明したフリーミアムの威力（2/2）

> *Zoomの40分制限がコロナ禍に最適な摩擦として有料転換を爆発的に加速した*
...
# ゲーム F2P — 転換率 0.5-3% でも成立する理由（2/2）

> *ゲームF2Pは鯨ユーザー0.5%が収益の50%を担う極端な集中モデルで成立する*
...
# Duolingo — ゲーミフィケーション型フリーミアム（2/2）

> *Duolingoのストリーク維持欲求が無料ユーザーを6ヶ月以上継続させる設計*
...
# 失敗パターン① — 無料版と有料版の価値差がない（2/2）

> *無料版と有料版の価値差が不明確な製品は転換率0.5%以下に沈む構造的失敗*
...
# 失敗パターン② — 無料ユーザーがコストを食い潰す（2/2）

> *無料ユーザーのCAC回収不能がキャッシュフロー危機を招く失敗の典型例*
...
> *Evernoteの制限強化が解約率急増を招いた—過剰制限が信頼を破壊する事例*
...
> *無料版の劣化と有料版の高額化の同時実行がブランド毀損を加速させた*
...
# 成功と失敗の分岐点 — 何が違うのか（2/2）

> *価値差の明確さ・無料ユーザーの収益貢献・転換摩擦の低さが成否を分ける*
...
# 3 つのゲートタイプ — Feature / Usage / Time（1/2）

> *ゲートの種類がユーザー体験と転換タイミングを決める*

<div class="fig">
<svg viewBox="0 0 800 400" style="display:block;margin:0 auto;display:block;width:100%;height:100%;max-width:100%;max-height:100%;margin:0 auto;letter-spacing:0;" xmlns="http://www.w3.org/2000/svg"><rect width="800" height="400" fill="#1a1a2e"/><text x="400" y="28" text-anchor="middle" fill="#ffffff" font-size="15" font-weight="bold">3 つのゲートタイプ比較</text><rect x="30" y="50" width="228" height="300" fill="#16213e" rx="8" stroke="#4a9eff" stroke-width="2"/><text x="144" y="85" text-anchor="middle" fill="#4a9eff" font-size="14" font-weight="bold">Feature Gate</text><text x="144" y="108" text-anchor="middle" fill="#aaaacc" font-size="11">機能制限</text><text x="144" y="140" text-anchor="middle" fill="#ffffff" font-size="11">基本機能は無料</text><text x="144" y="160" text-anchor="middle" fill="#ffffff" font-size="11">高度機能は有料</text><text x="144" y="195" text-anchor="middle" fill="#aaaacc" font-size="10">例: Slack ワークフロー</text><text x="144" y="213" text-anchor="middle" fill="#aaaacc" font-size="10">Zoom 管理機能</text><text x="144" y="250" text-anchor="middle" fill="#4a9eff" font-size="11">向く製品</text><text x="144" y="270" text-anchor="middle" fill="#aaaacc" font-size="11">B2B SaaS</text><text x="144" y="290" text-anchor="middle" fill="#aaaacc" font-size="11">コラボツール</text><rect x="286" y="50" width="228" height="300" fill="#16213e" rx="8" stroke="#51cf66" stroke-width="2"/><text x="400" y="85" text-anchor="middle" fill="#51cf66" font-size="14" font-weight="bold">Usage Gate</text><text x="400" y="108" text-anchor="middle" fill="#aaaacc" font-size="11">使用量制限</text><text x="400" y="140" text-anchor="middle" fill="#ffffff" font-size="11">一定量まで無料</text><text x="400" y="160" text-anchor="middle" fill="#ffffff" font-size="11">超過分は課金</text><text x="400" y="195" text-anchor="middle" fill="#aaaacc" font-size="10">例: Dropbox 2GB</text><text x="400" y="213" text-anchor="middle" fill="#aaaacc" font-size="10">SendGrid 100通/日</text><text x="400" y="250" text-anchor="middle" fill="#51cf66" font-size="11">向く製品</text><text x="400" y="270" text-anchor="middle" fill="#aaaacc" font-size="11">ストレージ・API</text><text x="400" y="290" text-anchor="middle" fill="#aaaacc" font-size="11">メール配信</text><rect x="542" y="50" width="228" height="300" fill="#16213e" rx="8" stroke="#f9a825" stroke-width="2"/><text x="656" y="85" text-anchor="middle" fill="#f9a825" font-size="14" font-weight="bold">Time Gate</text><text x="656" y="108" text-anchor="middle" fill="#aaaacc" font-size="11">期間制限</text><text x="656" y="140" text-anchor="middle" fill="#ffffff" font-size="11">評価期間後に</text><text x="656" y="160" text-anchor="middle" fill="#ffffff" font-size="11">判断を迫る</text><text x="656" y="195" text-anchor="middle" fill="#aaaacc" font-size="10">例: Adobe CC 7日</text><text x="656" y="213" text-anchor="middle" fill="#aaaacc" font-size="10">Salesforce 30日</text><text x="656" y="250" text-anchor="middle" fill="#f9a825" font-size="11">向く製品</text><text x="656" y="270" text-anchor="middle" fill="#aaaacc" font-size="11">Enterprise SaaS</text><text x="656" y="290" text-anchor="middle" fill="#aaaacc" font-size="11">高価格ツール</text></svg>
</div>
...
# 3 つのゲートタイプ — Feature / Usage / Time（2/2）

> *Feature・Usage・Timeの3ゲートタイプを組み合わせた設計が最高転換率を生む*
...
> *アップグレードへの動線を7ステップから3ステップに短縮する設計原則*
...
> *プログレスバー・使用量警告・限定プロンプトの3つが転換摩擦を下げる*
...
# PLG（Product-Led Growth）との関係（2/2）

> *PLGはフリーミアムを製品主導の成長エンジンに転換する上位概念*
...
# Expansion Revenue — アップセル設計（2/2）

> *アップセル設計で1ユーザーあたり収益を初回転換から3年で2.4倍に拡大する*
...
> *チャーン発生前30日に行動シグナルを検知し介入する予防的リテンション設計*
...
> *成功ユーザーの行動パターンを特定しオンボーディングで再現するCSの核心*
...
# 機械学習で転換を予測する — 行動データの活用（2/2）

> *行動データ200点の機械学習モデルが転換予測精度85%を達成した事例*
...
> *レート制限・デバイス制限・メールドメイン検証で悪用を抑制する技術手法*
...
> *機械学習による異常使用検知が悪用アカウントの95%を自動遮断する*
...
> *A/Bテストで課金フローの各ステップの離脱率を個別最適化する手順*
...
# フリーミアム適合度チェックリスト（2/2）

> *10項目チェックリストでフリーミアムの適合度を事前に定量評価できる*
...
> *フリーミアム経済学の理論的基盤となる一次文献と実証研究のリスト*
