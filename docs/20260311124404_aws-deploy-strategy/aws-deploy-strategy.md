...
<!--
ValidateService フックは最重要。ここでHTTPヘルスチェックを実行し、失敗時にCodeDeployが自動ロールバックを開始する仕組みを作ること。
-->
...
<!--
So What? Hooksで前後にLambdaを挟むことで、本番トラフィック切り替え前後の自動検証が実現できる。Alarmsに連携すると異常検知時に自動ロールバックが走る。
-->
...
<!--
So What? appspec.ymlのhooksにLambda関数を指定するだけで、デプロイパイプラインに自動検証を組み込める。設定はシンプルだが効果は大きい。
-->
...
<!--
CDK PipelinesはCodePipeline上に構築された高レベルAPIで、Stageを追加するだけで環境を増やせる。Self-mutating（パイプライン自体もCDKで管理）なので、パイプラインの変更も自動的に反映される。
-->
...
# デプロイ戦略ベストプラクティス10選：現場で効く実践知識を凝縮した
