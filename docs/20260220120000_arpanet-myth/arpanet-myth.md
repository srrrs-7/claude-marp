...
> *ARPANETの真実を一次資料から解体する全89スライドの構成*
...
> *Lickliderの夢は人機共生、ネットワークは手段に過ぎなかった*
...
# J.C.R. Licklider — "Man-Computer Symbiosis"の夢（2/2）（1/2）
...
# J.C.R. Licklider — "Man-Computer Symbiosis"の夢（2/2）（2/2）

> *銀河間ネットワーク構想がARPANETの思想的起源を作った*
...
> *IPTOの構想は研究者のコラボレーション促進が第一目標だった*
...
# IPTO設立と「銀河間コンピュータネットワーク」構想（2/2）（1/2）
...
# IPTO設立と「銀河間コンピュータネットワーク」構想（2/2）（2/2）

> *タイムシェアリングの地理的拡張がARPANETの根本動機だった*
...
# 本当の動機: リソース共有とコスト削減（2/2）（2/2）

> *一次資料は核戦争でなく研究者の不便解消が動機と証明する*
...
# ポール・バランが本当に研究したもの（RAND、1960〜64年）（1/2）（1/2）
...
# ポール・バランが本当に研究したもの（RAND、1960〜64年）（1/2）（2/2）

> *バランの研究動機は核攻撃後の軍指揮系統維持—民間転用ではない*
...
# ポール・バランが本当に研究したもの（RAND、1960〜64年）（2/2）（1/2）
...
# ポール・バランが本当に研究したもの（RAND、1960〜64年）（2/2）（2/2）

> *AT&Tに否定され棚上げになったバランの提案はARPANETと無関係*
...
# バランの研究と軍 — ARPANETとは別物（1/2）（2/2）

> *バランの論文は参照されたがARPANETの設計書にはなっていない*
...
# バランの研究と軍 — ARPANETとは別物（2/2）（2/2）

> *バラン自身が否定し続けた「ARPANETの設計者」という誤解*
...
# ドナルド・デービス — パケット交換の独立発明（英国NPL）（1/2）（1/2）
...
# ドナルド・デービス — パケット交換の独立発明（英国NPL）（1/2）（2/2）

> *デービスが独立発明した事実が核戦争必須論を崩す最強の証拠*
...
# ドナルド・デービス — パケット交換の独立発明（英国NPL）（2/2）（1/2）
...
# ドナルド・デービス — パケット交換の独立発明（英国NPL）（2/2）（2/2）

> *デービスは独立にパケット交換を発明し、BARSを先取りしていた*
...
# IMP（Interface Message Processor）（1/2）（1/2）
...
# IMP（Interface Message Processor）（1/2）（2/2）

> *IMPは汎用ルーターの原型、パケット交換を物理的に実現した*
...
# IMP（Interface Message Processor）（2/2）（1/2）
...
# IMP（Interface Message Processor）（2/2）（2/2）

> *IMP1台$65,000の相互運用装置—核耐性ではなく接続性のため*
...
# 1969年10月29日 — 「lo」: 最初の送信と最初のクラッシュ（1/2）（1/2）
...
# 1969年10月29日 — 「lo」: 最初の送信と最初のクラッシュ（1/2）（2/2）

> *「login」→「lo」でクラッシュ—歴史的初送信は失敗で幕を開けた*
...
# 1969年10月29日 — 「lo」: 最初の送信と最初のクラッシュ（2/2）（1/2）
...
# 1969年10月29日 — 「lo」: 最初の送信と最初のクラッシュ（2/2）（2/2）

> *最初の送信「lo」はクラッシュで終わり、失敗が歴史に刻まれた*
...
# 核攻撃耐性は「副産物」だったか — 証拠を読む（1/2）（2/2）

> *設計文書にも初期4ノードにも核耐性の痕跡は存在しない*
...
# 核攻撃耐性は「副産物」だったか — 証拠を読む（2/2）（2/2）

> *Vint Cerfが「副産物をラベルにすり替えた」と神話の正体を看破*
...
# 当時のARPA内部文書が示す優先事項（2/2）（2/2）

> *BBN仕様書に軍事要件ゼロ—一次資料を読めば神話は崩壊する*
...
# 「Where Wizards Stay Up Late」(1996)が明かした事実（1/2）（1/2）
...
# 「Where Wizards Stay Up Late」(1996)が明かした事実（1/2）（2/2）

> *数十人への直接取材と内部文書精査が神話を初めて系統的に否定*
...
# 「Where Wizards Stay Up Late」(1996)が明かした事実（2/2）（1/2）

> *1996年の書籍が初めて一次資料に基づく事実を広く伝えた*
...
# 「Where Wizards Stay Up Late」(1996)が明かした事実（2/2）（2/2）

> *学術的否定後もポップカルチャーで神話が生き残り続けた*
...
# メール — 誰も計画していなかったキラーアプリ（1971年）（1/2）（1/2）
...
# メール — 誰も計画していなかったキラーアプリ（1971年）（1/2）（2/2）

> *「@」記号の選択理由—「他の用途がほぼない文字だったから」*
...
# メール — 誰も計画していなかったキラーアプリ（1971年）（2/2）（1/2）
...
# メール — 誰も計画していなかったキラーアプリ（1971年）（2/2）（2/2）

> *メールは誰も計画しなかったが最初のキラーアプリになった*
...
# RFC文化 — 「お願い」から始まる標準化（1/2）（2/2）

> *「コメント求む」という謙虚な姿勢が開かれた標準化文化を生んだ*
...
# RFC文化 — 「お願い」から始まる標準化（2/2）（2/2）

> *RFCの「お願い」文化が権威なき標準化プロセスを生んだ*
...
# TCP/IP — サーフとカーンの革命（1974年）（1/2）（1/2）
...
# TCP/IP — サーフとカーンの革命（1974年）（1/2）（2/2）

> *Cerf & Kahnの1974年論文がネットワークのネットワーク実現を宣言*
...
# TCP/IP — サーフとカーンの革命（1974年）（2/2）（1/2）
...
# TCP/IP — サーフとカーンの革命（1974年）（2/2）（2/2）

> *1983年1月1日のNCP→TCP/IP切り替えがインターネット誕生日*
...
# NSFNETへの移行 — 軍用から学術・商業へ（1/2）（2/2）

> *NSFNETがバックボーンを構築し研究者接続を数十倍に拡大した*
...
# NSFNETへの移行 — 軍用から学術・商業へ（2/2）（2/2）

> *NSFNETへの移行が軍用ネットワークを公共インフラに変えた*
