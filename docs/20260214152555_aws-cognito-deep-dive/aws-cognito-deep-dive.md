...
> *Overview・UserPool・認証フロー・IdentityPool・Federationの前半5章を解説*
...
> *Security・AWS統合・Operations・Pricing・Best Practicesの後半5章を解説*
...
> *CognitoはUserPool（JWT発行）とIdentityPool（STS変換）で認証→認可を完結*
...
<!-- _class: invert fit-88 -->
# User Pool vs Identity Pool（2/2）

> *Identity PoolがJWT→STS変換でIAMロールによるAWSリソースアクセスを実現*
...
> *PublicクライアントはPKCE必須、ConfidentialクライアントはSecretありで認証設定を分離*
...
> *USER_SRP_AUTHはパスワードをネットワーク上で送信しない最も安全な認証フロー*
...
> *WebAuthn/FIDO2パスキーがフィッシング耐性を確保しPasswordless認証の本命として登場*
...
> *認証済みはIAMロール権限でフルアクセス、未認証ゲストは限定AWSリソースのみ許可*
...
> *デフォルトロール・ルールベース・トークンベース・細粒度の4段階でIAMロールを動的割り当て*
...
> *Social/SAML/OIDCの3方式で主要IdPを網羅し2024年2月からSAMLリクエスト署名も対応*
...
> *2026年1月リリースのInbound Federation TriggerでリアルタイムSAML/OIDC属性変換が可能*
...
> *Pre Token Generation v2/v3でアクセストークンのスコープ・クレームをLambdaで完全カスタマイズ*
...
> *Refresh Token Rotationで旧トークン自動無効化しリプレイ攻撃リスクを大幅低減*
...
> *SMS/TOTP/Email MFAの3方式に加えパスキーは追加MFA不要で最も強固な認証を実現*
...
> *Plusティアの適応認証がデバイス・IP・位置情報×漏洩クレデンシャル検出でリスクを自動判定*
...
> *WAF Web ACLのBot Control・レート制限・地理フィルタリングでCognito前段の攻撃を完全遮断*
...
> *SOC/HIPAA/PCI DSS/ISO/FedRAMP取得でエンタープライズ・医療・金融・政府案件に対応*
...
> *ALBがOIDC認証をオフロードしアプリ側実装不要でAppSyncはCognito認可モードを直接設定*
...
# Lambda Triggers 一覧（1/2）

> *Lambda Trigger 14種類の適切な使い分けが実装品質の差を生む*
...
# Lambda Triggers 一覧（2/2）

> *トークンカスタマイズ・フェデレーション・メッセージ・Migrate Userトリガーが後半の主要Trigger*
...
> *Identity Pool経由のSTS一時認証情報でS3ユーザー専用フォルダとDynamoDB行レベル制御を実現*
...
> *PrivateLink（2025年11-12月）でインターネット不使用の認証を完全プライベート化*
...
> *CloudWatch Metrics・CloudTrail・脅威保護ログで認証エラー率からAPI呼び出しまで完全監視*
...
> *Migrate User Triggerによるオンデマンド移行が最も安全なユーザー移行戦略として推奨*
...
> *Essentials $0.015/MAUが機能・コストのバランス最優先、Plus $0.020/MAUで脅威保護を追加*
...
> *リフレッシュトークン長寿命化とTOTP優先でMAUカウント削減とSMSコスト削減を実現*
...
> *SRP認証・Passwordless・Token Rotation・Resource Indicatorsの4要素がCognito設計の必須基準*
...
> *WAF必須適用・Access Token 15-60分・PKCE必須・サーバーサイド検証の5大セキュリティルール*
...
> *Client Secret露出・IDトークンAPI認可・トークン検証省略・PASSWORD_AUTH使用の5大アンチパターン撲滅*
...
> *Cognito公式ドキュメント・APIリファレンス・料金ページの基本リソース集*
...
> *Passwordless実装・アクセストークンカスタマイズ・WAF設定のAWSセキュリティブログ集*
