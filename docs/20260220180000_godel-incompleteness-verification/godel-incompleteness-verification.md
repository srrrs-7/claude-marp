...
> *汎用停止判定アルゴリズムの不存在がソフトウェア検証の限界を画す*
...
> *形式証明ツール自体の無矛盾性も外部体系に依存し完全保証は不可能*
