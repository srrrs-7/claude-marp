...
> *Agent Teamsの概念・コンポーネント・動作原理を10章で完全解説*
...
> *アーキテクチャからコスト最適化・ユースケースまでの後半5章構成*
...
# なぜAgent Teamsなのか？（1/2）

> *単一エージェントの限界をマルチエージェントで突破する理由*
...
# なぜAgent Teamsなのか？（2/2）

> *マルチエージェントは専門コンテキスト分離と並列実行で単一AIの限界を超える*
...
> *Agent Teamsは独立コンテキスト×共有タスクリスト×直接メッセージングで協調動作*
...
> *コンテキスト非共有のAgent Teamsが単一比で最大10倍の速度を実現*
...
# TeamCreate / TeamDelete（1/2）

> *TeamCreateで生成されるディレクトリ構造がチーム全体の基盤になる*
...
# TeamCreate / TeamDelete（2/2）

> *TeamDeleteは全メンバーをシャットダウン後でないと削除できないライフサイクル制約*
...
> *TaskUpdateのステータス管理とdependencyで並列タスクの安全な実行順序を制御*
...
> *ファイルロックによる競合防止とTaskListでチームメイトが自律的に次作業を発見*
...
> *5つのメッセージタイプを使い分けることで通信コストを最小化できる*
...
> *Explore・Plan・Codeの3ビルトインタイプが95%のユースケースをカバー*
...
> *カスタムエージェントでチーム固有のペルソナと権限を設計できる*
...
> *モデル選択・権限モード・MCPアクセス・スキルプリロードでエージェントを完全カスタマイズ*
...
> *Team Leadがチーム作成・タスク分解・統合を担いTeammatesが独立実行に特化*
...
> *Teammatesはリーダー履歴なしでもCLAUDE.md自動ロードで独立実行後idle復帰*
...
> *依存関係の明示的定義がウォーターフォールを安全に並列化する鍵*
...
# Plan Approvalワークフロー（1/2）

> *Plan Approvalでリスクの高い変更を実行前に人間がレビューできる*
...
# Plan Approvalワークフロー（2/2）

> *Plan Approval→承認→実装→承認却下→修正のループでアーキテクチャ変更を安全に実施*
...
> *Delegateモードはリーダーを実装禁止にしコーディネーション専任にする全体最適化モード*
...
> *Delegateモードで全体最適に集中したリーダーがSpawn・タスク管理のみを行う*
...
> *split paneモードが視認性と操作性の最適バランスを提供する*
...
> *リサーチ並列化・機能分担・並列デバッグ・クロスレイヤー変更の5ユースケースで生産性最大化*
...
> *16エージェントが$20,000・2,000セッションで10万行Cコンパイラを3アーキテクチャ対応で構築*
...
> *十分なコンテキスト提供と適切なタスクサイズ設定がAgent Teams成功の2大原則*
...
> *1チームメイトあたり5-6タスクが理想で同一ファイルの複数担当は上書き競合を発生*
...
> *計画フェーズ（~10kトークン低コスト）で分解確認後に高コスト実行フェーズへ移行*
...
> *権限事前設定と中断最小化でスムーズ実行、OpusリーダーとSonnetチームメイトでコスト最適化*
...
> *リサーチ系タスクから検証開始し並列化メリットが明確な作業のみに限定してコスト管理*
...
> *セッション復元不可・1セッション1チームの制約を設計前に把握する*
...
> *4コンポーネント×7プリミティブ×3フェーズ×Plan Firstで大規模開発を加速*
