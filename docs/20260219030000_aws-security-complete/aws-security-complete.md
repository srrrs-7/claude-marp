...
> *SCP(組織全体の境界)とPermission Boundary(個別の上限)は適用スコープが異なる*
...
> *ルートアカウント不使用・MFA強制・最小権限・アクセスキー不使用の4原則*
...
> *Secrets Manager(自動ローテーション・高コスト)とParameter Store(手動・低コスト)を用途で選択*
...
> *SSE-S3(AWS管理)/SSE-KMS(監査可能)/SSE-C(顧客キー)/CSE(クライアント暗号化)の4択*
...
> *CloudHSMはFIPS 140-2 Level 3準拠の専用HSM、KMSより高い規制対応が可能*
...
> *SGはステートフル(戻りトラフィック自動許可)、NACLはステートレス(両方向設定必須)*
...
> *Shield Standard(無料・自動)とShield Advanced(月$3000・DRT対応)を用途で選択*
...
> *GuardDuty Findingsの重要度High/Medium/Lowと対応アクションをマッピング*
...
> *Trusted Advisorのセキュリティチェック14項目で未設定のMFA・公開SG等を検出*
...
> *Audit Managerは規制フレームワークの証拠を自動収集してコンプライアンスを効率化*
...
> *Inspector v2はEC2/Lambda/ECRの脆弱性をCVSSスコアで優先度付けして継続スキャン*
...
> *AWS Artifactで規制当局向けコンプライアンスレポートをオンデマンド取得可能*
...
> *Security Pillarの7設計原則が試験の設計問題で判断軸として機能する*
...
> *共有責任モデルはIaaS/PaaS/SaaSでAWSとユーザーの境界が移動する*
...
> *AWSのIRフレームワーク6フェーズで準備→検出→分析→封じ込め→根絶→復旧を実施*
...
> *RTO/RPO要件に基づいてBackup・Pilot Light・Warm Standby・Multi-Siteを選択*
...
> *試験に出る制限値(GuardDuty 30日Trial・Shield $3000/月等)を一覧で暗記*
...
> *サービス選択問題は要件キーワードとサービスのマッピング表で即答できる*
...
> *A〜KのキーワードでAWSセキュリティサービスを横断的に索引引きする*
...
> *L〜ZのキーワードでSNI/TLS/VPC等の技術用語とAWSサービスを紐付ける*
...
> *引っかけ問題の9割は「似たサービスの使い分け」と「評価順序の誤解」が根本*
