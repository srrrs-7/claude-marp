...
> *12章構成でデータ管理からBedrock MLOpsまで網羅*
...
> *8フェーズが循環—Pipelinesが全フェーズを自動オーケストレーション*
...
> *SageMaker Pipelinesが全工程のオーケストレーション中核*
...
> *Studio+Pipelines+Registryで学習〜本番を一気通貫管理*
...
> *Experimentで全パラメータ・メトリクスを自動追跡・比較*
...
> *Raw→Curated→Featuresの3層構造で品質と速度を両立*
...
> *推論はオンライン(<10ms)、学習はオフライン(S3)を使い分ける*
...
> *Augmentationで学習データを5〜10倍に増やし汎化性能を向上*
...
> *SMOTE+クラス重み付けでFスコアを平均15pt改善できる*
...
> *300以上の変換がノーコードで実行可能—前処理時間を80%削減*
...
# データ変換パターン — SageMaker Processing（コード例）
...
# 検証ルール設計パターン — QualityCheckStep（コード例）
...
> *統計的手法+MLで正常範囲を学習—99%の外れ値を自動フラグ*
...
> *マネージド学習環境でインフラ管理ゼロ—スポットで最大90%削減*
...
> *任意のフレームワークをDockerで持ち込み—完全な柔軟性を確保*
...
> *Data/Model Quality+Bias+Explainability—全て有効化が推奨*
...
> *閾値ベースのEventBridgeルールで人手を介さず再学習起動*
...
# MLOps IaC パターン — AWS CDK + SageMaker Pipelines（コード例）
